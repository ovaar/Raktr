 ...
 * @brief Implementation of geometry rendering pass.
 */
...
namespace raktr::render
{
    ...
    void GeometryPass::execute(PassContext& ctx)
    {
        ...
        spdlog::info("GeometryPass: Drew {}/{} instances ({:.1f}% culled)",
                     drawn_count,
                     _instance_count,
                     culling_rate);
    }

    void GeometryPass::on_viewport_resize([[maybe_unused]] uint32_t width, [[maybe_unused]] uint32_t height)
    {
        // Nothing to do - render targets are managed by PassContext
    }

    void GeometryPass::update_visibility([[maybe_unused]] const std::vector<bool>* visibility)
    {
