/*!
 * @file wgpu_frame_resources.cpp
 * @brief Implementation of WgpuFrameResources.
 */

#include "wgpu_frame_resources.h"
#include <cassert>

namespace raktr::render::backend::wgpu
{

    WgpuFrameResources::WgpuFrameResources(WGPUDevice device, uint32_t num_frames)
    ...
    WgpuFrameResources::~WgpuFrameResources()
    {
    ...
    WgpuFrameResource& WgpuFrameResources::current()
    {
        return _frames[_current_index];
    }

    const WgpuFrameResource& WgpuFrameResources::previous() const
    {
    ...
    void WgpuFrameResources::advance_frame()
    {
        _current_index = (_current_index + 1) % _frames.size();
        _frame_counter++;
    }

    void WgpuFrameResources::recreate_resources(uint32_t width, uint32_t height)
    {
    ...
    void WgpuFrameResources::wait_idle()
    {
