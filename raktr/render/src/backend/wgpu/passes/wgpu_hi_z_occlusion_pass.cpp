...
#include "wgpu_hi_z_occlusion_pass.h"
#include <spdlog/spdlog.h>

namespace raktr::render::backend::wgpu
{

    WgpuHiZOcclusionPass::WgpuHiZOcclusionPass(
        ...
        : _hi_z_buffer(hi_z_buffer), _aabbs(aabbs), _view_projection(view_projection), _visibility_results(visibility_results)
    {
        ...
        {
            spdlog::warn("WgpuHiZOcclusionPass: Hi-Z buffer is null, culling will be disabled");
        }
    }

    void WgpuHiZOcclusionPass::execute([[maybe_unused]] WgpuPassContext& ctx)
    {
        ...
        else
        {
            ...
            spdlog::warn("WgpuHiZOcclusionPass: Visibility test failed, marking all objects visible");
            if (_visibility_results && _aabbs)
            {
                _visibility_results->assign(_aabbs->size(), true);
            }
    ...
    void WgpuHiZOcclusionPass::on_viewport_resize([[maybe_unused]] uint32_t width, [[maybe_unused]] uint32_t height)
    {
        // Hi-Z buffer will be recreated by the application, nothing to do here
    }

    void WgpuHiZOcclusionPass::update(const std::vector<occlusion::AABB>* aabbs, const glm::mat4& view_projection)
    {
        _aabbs           = aabbs;
