...
namespace raktr::render::backend::wgpu
{
     ...
     * @example
     * @code
     * graph.add_pass(WgpuGeometryPass{
     *     vertex_buffer,
     *     index_buffer,
     *     instance_buffer,
     *     &visibility_results,
    ...
    public:
        ...
        WgpuGeometryPass(const WgpuGeometryPass&)            = delete;
        WgpuGeometryPass& operator=(const WgpuGeometryPass&) = delete;
        WgpuGeometryPass(WgpuGeometryPass&&)                 = default;
        WgpuGeometryPass& operator=(WgpuGeometryPass&&)      = default;

        void                           execute(WgpuPassContext& ctx);
        void                           on_viewport_resize(uint32_t width, uint32_t height);
        [[nodiscard]] std::string_view name() const
        {
            return "WgpuGeometryPass";
        }
