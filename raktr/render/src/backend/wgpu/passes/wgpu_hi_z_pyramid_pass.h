...
namespace raktr::render::backend::wgpu
{
    ...
    class WgpuHiZPyramidPass final
    {
    public:
        ...
        ~WgpuHiZPyramidPass() = default;

        // Non-copyable, movable
        WgpuHiZPyramidPass(const WgpuHiZPyramidPass&)            = delete;
        WgpuHiZPyramidPass& operator=(const WgpuHiZPyramidPass&) = delete;
        WgpuHiZPyramidPass(WgpuHiZPyramidPass&&)                 = default;
        WgpuHiZPyramidPass& operator=(WgpuHiZPyramidPass&&)      = default;

        void                           execute(WgpuPassContext& ctx);
        void                           on_viewport_resize(uint32_t width, uint32_t height);
        [[nodiscard]] std::string_view name() const
        {
            return "WgpuHiZPyramidPass";
        }
