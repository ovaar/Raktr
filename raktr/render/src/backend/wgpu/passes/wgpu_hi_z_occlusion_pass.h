...
namespace raktr::render::backend::wgpu
{
    ...
    class WgpuHiZOcclusionPass final
    {
    public:
        using ContextType = WgpuPassContext; // Required for type erasure

        WgpuHiZOcclusionPass(
            occlusion::HiZBuffer*               hi_z_buffer,
            const std::vector<occlusion::AABB>* aabbs,
            const glm::mat4&                    view_projection,
            std::vector<bool>*                  visibility_results);

        ~WgpuHiZOcclusionPass() = default;

        // Non-copyable, movable
        WgpuHiZOcclusionPass(const WgpuHiZOcclusionPass&)            = delete;
        WgpuHiZOcclusionPass& operator=(const WgpuHiZOcclusionPass&) = delete;
        WgpuHiZOcclusionPass(WgpuHiZOcclusionPass&&)                 = default;
        WgpuHiZOcclusionPass& operator=(WgpuHiZOcclusionPass&&)      = default;

        void                           execute(WgpuPassContext& ctx);
        void                           on_viewport_resize(uint32_t width, uint32_t height);
        [[nodiscard]] std::string_view name() const
        {
            return "WgpuHiZOcclusionPass";
        }
